...
public class ObstacleAvoidanceParallel : MonoBehaviour {
    ...
    {
        Vector3 leftRayPosition = transform.position - transform.right;
        Vector3 rightRayPosition = transform.position + transform.right;
        RaycastHit hit;
        // Only detect layer 8 (Obstacles)
        int layerMask = 1 << 8;
        // Check that the vehicle hit with the obstacles within
        // it's minimum distance to avoid
        if (Physics.Raycast(leftRayPosition, transform.forward, out hit, minimumDistToAvoid, layerMask))
        {
            Debug.DrawRay(leftRayPosition, transform.forward, Color.red);
            Vector3 hitNormal = hit.normal;
            hitNormal.y = 0.0f; // Don't want to move in Y-Space
            dir = transform.forward + hitNormal * force;
        }
        else if (Physics.Raycast(rightRayPosition, transform.forward, out hit, minimumDistToAvoid, layerMask))
        {
            Debug.DrawRay(rightRayPosition, transform.forward, Color.red);
            Vector3 hitNormal = hit.normal;
            hitNormal.y = 0.0f; // Don't want to move in Y-Space
            dir = transform.forward + hitNormal * force;
        }
        ...
        {
            Debug.DrawRay(leftRayPosition, transform.forward, Color.green);
            Debug.DrawRay(rightRayPosition, transform.forward, Color.green);
        }
    }
